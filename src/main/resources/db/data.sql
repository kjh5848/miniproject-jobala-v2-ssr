...
--댓글 더미
Insert Into reply_tb(id, user_id, board_id, comment, created_at)
values (1, 2, 1, '꼰대임?', now()),             --cos2이 1번 게시물에 댓글을 작성했다.
       (2, 3, 1, '넌 몇년차인데?ㅋㅋ', now()),       --cos3가 1번 게시물에 댓글을 작성했다.
       (3, 1, 2, '관심 받고 싶니?', now()),        --cos1이 2번 게시물에 댓글을 작성했다.
       (4, 3, 2, '너가 주면 난 내 전재산 준다', now()), --cos3가 2번 게시물에 댓글을 작성했다.
       (5, 2, 3, '뭐지', now()),               --cos3가 3번 게시물에 댓글을 작성했다.
       (1, 2, 3, '아재요?', now()),             -- cos2가 게시물 번호 1에 댓글을 남겼습니다.
       (2, 3, 4, '나이는 몇 살이세요? ㅎㅎ', now()),   -- cos3가 게시물 번호 1에 댓글을 남겼습니다.
       (3, 1, 5, '신경쓰여요?', now()),           -- cos1이 게시물 번호 2에 댓글을 남겼습니다.
       (4, 3, 6, '주시면 다 줄게요', now()),        -- cos3가 게시물 번호 2에 댓글을 남겼습니다.
       (5, 2, 7, '뭐라고요?', now()),            -- cos3가 게시물 번호 3에 댓글을 남겼습니다.
       (6, 1, 8, '힘내세요!', now()),
       (7, 2, 8, '오늘도 화이팅!', now()),
       (8, 3, 9, '바닥부터 다시 시작해보세요', now()),
       (9, 1, 9, '한걸음 먼저 나아가보세요', now()),
       (10, 2, 10, '힘내세요!', now()),
       (11, 3, 10, '당신은 충분히 좋아요', now());
